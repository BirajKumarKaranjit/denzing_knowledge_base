...
RELEVANCE_CHECK_SYSTEM_PROMPT = (
    ...
    "4. OUT_OF_DOMAIN\n"
    "   The query asks for information that cannot be retrieved or computed\n"
    "   using the provided database schema.\n\n"
    "   A query must be classified as OUT_OF_DOMAIN only when the requested\n"
    "   information cannot be mapped to any table, column, or computable\n"
    "   aggregation based on the schema context, and the classifier is\n"
    "   confident that no reasonable computation from the available fields\n"
    "   can produce the requested result.\n\n"
    "   ### ENTITY VS ATTRIBUTE RULE ###\n"
    "   Proper nouns such as people, organisations, products, or other named\n"
    "   entities may exist as values inside text columns. The presence of a\n"
    "   name alone does NOT make a query OUT_OF_DOMAIN.\n\n"
    "   The requested ATTRIBUTE or METRIC must also correspond to:\n"
    "   - a column that exists in the schema, OR\n"
    "   - a value that can reasonably be computed from existing numeric columns\n"
    "     using standard aggregations such as count, sum, average, min, max,\n"
    "     or simple ratios and joins that are supported by the schema.\n\n"
    "   If the query asks for an attribute, property, or metric that does not\n"
    "   exist in any column and cannot be derived from available columns,\n"
    "   the query should be classified as OUT_OF_DOMAIN — but only when the\n"
    "   classifier is certain of this mapping failure.\n\n"
    "   IMPORTANT BIAS RULE:\n"
    "   When there is any uncertainty about whether the requested information\n"
    "   can be retrieved or computed from the schema, DO NOT classify the query\n"
    "   as OUT_OF_DOMAIN. Bias toward SQL_RELEVANT: prefer passing ambiguous\n"
    "   queries through to the SQL stage rather than rejecting potentially\n"
    "   valid requests. Only label as OUT_OF_DOMAIN when you are confident the\n"
    "   schema cannot satisfy the request.\n\n"
    "   Examples of valid OUT_OF_DOMAIN signals:\n"
    "   - the requested metric or attribute is not present and cannot be computed\n"
    "   - the request requires external data not represented in the schema\n\n"
    "   IMPORTANT: Even if a proper noun could exist in the database,\n"
    "   the query is OUT_OF_DOMAIN only if the requested attribute or metric\n"
    "   cannot be retrieved or calculated from the schema and the classifier\n"
    "   is confident in that determination.\n\n"
