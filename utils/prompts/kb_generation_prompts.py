...
RELEVANCE_CHECK_SYSTEM_PROMPT = (
    ...
    "4. OUT_OF_DOMAIN\n"
    "   The query asks for information that CANNOT — even in principle — be retrieved\n"
    "   or computed from the provided DDL schema.\n\n"
    "   ### DDL-First Reasoning Rule (apply BEFORE classifying OUT_OF_DOMAIN)\n"
    "   Scan every table and column in the schema. Ask:\n"
    "     a) Could any column plausibly store the requested data?\n"
    "     b) Could any join/aggregation across existing columns produce the answer?\n"
    "   If YES to either → SQL_RELEVANT. Do not classify OUT_OF_DOMAIN.\n\n"
    "   ### ENTITY VS ATTRIBUTE RULE\n"
    "   Named entities (people, teams, organisations, products) may exist as values\n"
    "   inside text columns. Never reject a query solely because it names a real-world\n"
    "   entity. The requested ATTRIBUTE or METRIC must also be missing from the schema.\n\n"
    "   OUT_OF_DOMAIN only when BOTH conditions are true:\n"
    "     1. No column stores or could plausibly store the requested data.\n"
    "     2. The answer cannot be derived from any aggregation of existing columns.\n\n"
    "   True OUT_OF_DOMAIN examples (schema has no remotely related columns):\n"
    '   - "What is the weather in Tokyo today?"\n'
    '   - "Write me a poem about data"\n'
    '   - "What is the meaning of life?"\n\n'
    "   NOT OUT_OF_DOMAIN — queries that reference real-world entities but map to schema columns:\n"
    "   - Any question about a named entity (person, org, team, product, place) where\n"
    "     the schema has columns that could store names, identifiers, or related metrics.\n"
    "   - Any question about a time-bound event or result where the schema has date,\n"
    "     period, or status columns.\n"
    "   - Any question asking for a ranking, winner, or top performer where the schema\n"
    "     has numeric or categorical outcome columns.\n\n"
    "   TRUE OUT_OF_DOMAIN signals (schema structurally cannot answer):\n"
    "   - The requested information requires real-time external data (live prices, weather).\n"
    "   - The requested metric has no corresponding column type anywhere in the schema.\n"
    "   - The query is about general world knowledge with zero mapping to any table.\n\n"
    "   BIAS RULE: When uncertain, choose SQL_RELEVANT. You must be 100% certain the\n"
    "   schema cannot satisfy the request before returning OUT_OF_DOMAIN.\n\n"
