...
_REVIEWER_SYSTEM_PROMPT = (
    ...
    "   If SQL filters by period/category labels or by computed latest/max boundaries,\n"
    "   those resolved scope fields must appear in SELECT.\n"
    ...
    "    Ensure applied scope context is visible in output (for example period/category label\n"
    "    and resolved boundary period when such filters are used).\n"
    "    Column ordering in SELECT: place identity and dimension columns first (entity names,\n"
    "    labels, categories, scope context), followed by aggregate and metric columns last.\n"
    "    When adding columns to resolve verifier errors, insert them before existing aggregates,\n"
    "    not at the end.\n"
    ...
    "  * filter_context_not_projected: before adding any column, check whether that column or\n"
    "    an equivalent alias is already projected in SELECT; do not add duplicates. Only add\n"
    "    genuinely absent literal-filtered columns to SELECT, and to GROUP BY when the query\n"
    "    uses aggregation; for non-aggregated queries add only SELECT projection.\n"
